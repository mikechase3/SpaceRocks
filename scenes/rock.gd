...
func start(_position: Vector2, _velocity: Vector2, _size: float) -> void:
	...
	$Explosion/Sprite2D.scale = Vector2.ONE * 0.75 * size  # TODO: might ref wrong node?

func explode():
	$CollisionShape2D.set_deferred("disabled", true)
	$Sprite2D.hide()
	$Explosion/AnimationPlayer.play("Explosion")  # lowercase in slides fysa.
	$Explosion/Sprite2D.show()
